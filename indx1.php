...
?>
		...
		<script type="text/javascript">
			...
			function user_register(user_id, user_name , email) {
		...
		if (count($user) > 0) {
			...
			foreach ($user as $row) {
				...
				if (count($finger) == 0) {
					...
					$register = "<a href='finspot:FingerspotReg;$url_register' class='btn btn-xs btn-primary' onclick=\"user_register('".$row['user_id']."','".$row['user_name']."', '".$row['email']."')\">Register</a>";

				} else {
					
					$verification = "<a href='finspot:FingerspotVer;$url_verification' class='btn btn-xs btn-success'>Sign in</a>";
